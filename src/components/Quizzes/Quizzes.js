...
const Quizzes = ({ quizzes }) => {
    ...
    return (
        <div className=''>
            <div className="sm:w-3/5  auto-w-sm bg-white-100 rounded-lg shadow-md  border-solid border-2 border-sky-500  mt-5 ">
                <Link to={`quiz/${id}`}>
                   <img src={logo} alt="" className=' p-8 bg-black rounded-t-lg' />
                </Link>
                <div class="px-5 pb-5">
                    <Link to={`quiz/${id}`}>
                        <h5 className="text-3xl font-bold text-sky-900 " >{name}</h5>
                    </Link>
                    <span className="text-xl font-semibold tracking-tight text-orange-700 ">TotalQuiz: {total}</span>
